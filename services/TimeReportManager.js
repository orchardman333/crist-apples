...
module.exports = {
  DailyTime: function(req,res) {
    ...
    .then(results => {
      ...
      return query.queryOnly(results.connection, 'SELECT ROUND(HOUR(TIMEDIFF(time_table.`Time Out`, time_table.`Time In`)) + MINUTE(TIMEDIFF(time_table.`Time Out`, time_table.`Time In`))/60,3) AS hours, time_table.`Employee ID` AS employeeId FROM time_table INNER JOIN employee_table ON employee_table.`Employee ID`=time_table.`Employee ID` WHERE DATE(`Time In`) = DATE(?) AND employee_table.`Department ID` IN ? ORDER BY time_table.`Employee ID`,time_table.`Time In` ASC', [req.body.date, [req.body.departmentIds]]);
    })
    .then(results => {
